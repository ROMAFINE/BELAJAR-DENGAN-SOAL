...
function saveParticipantProgress() {
  ...
  if (session.serverAuth) {
    apiRequest("/api/participant/progress", {
      method: "POST",
...
async function apiRequest(path, options = {}) {
  ...
  if (!response.ok) {
    const message = payload.error || "Request gagal.";
    throw new Error(message);
  }
  return payload;
}

async function secureAdminLogin(password) {
...
async function secureParticipantLogin(name, password) {
  return apiRequest("/api/auth/participant", {
    method: "POST",
...
function setRoute(nextRoute) {
  ...
  if (nextRoute === "admin" && !(isAdmin && session?.role === "admin")) {
    window.alert("Admin harus login lewat server agar password tidak bocor di frontend.");
    nextRoute = "dashboard";
  }
  route = nextRoute;
...
async function renderLeaderboard() {
  ...
  if (session?.serverAuth) {
    try {
      ...
      renderLeaderboardRows(payload.records || [], currentId);
      return;
    } catch (error) {
      $("#leaderboardTable").innerHTML = `
        <article class="review-card">
          <span class="tag">Server</span>
          <h2>Leaderboard server belum tersedia.</h2>
          <p>Jalankan server publik agar data peserta tersimpan aman.</p>
        </article>
      `;
      return;
    }
...
function wireEvents() {
  $("#participantLogin").addEventListener("submit", async (event) => {
    ...
    try {
      const payload = await secureParticipantLogin(name, password);
      session = {
        role: "participant",
      ...
      isAdmin = false;
      localStorage.removeItem(ADMIN_MODE_KEY);
      applyParticipantProgress(payload.progress || {});
      syncAppVisibility();
      renderPlacement();
      renderResults();
      refreshAccessViews();
      ...
      if (window.location.protocol !== "file:") {
        window.alert(`Login peserta gagal: ${error.message}`);
        return;
      }
    }
  ...
  $("#adminLogin").addEventListener("submit", async (event) => {
    ...
    try {
      const payload = await secureAdminLogin(password);
      ...
      window.alert(`Login admin gagal: ${error.message}. Jalankan server agar password admin aman dan tidak bocor di frontend.`);
      return;
    }
  });
...
function init() {
  ...
  if (session?.role === "participant") {
    session.id = session.id || leaderboardIdForName(session.name);
    saveSession();
    if (!session.serverAuth) loadParticipantProgress(session.id);
  }
  syncAppVisibility();
  renderBasic();
  renderDashboard();
